...
PanelWindow {
    ...
    property int wheelPendingDir: 0
    ...
    Timer {
        ...
        repeat: false
        onTriggered: {
            if (!panel.ready || panel.wheelSteps <= 0 || panel.wheelPendingDir === 0) return

            panel.selectAdjacent(panel.wheelPendingDir)
            panel.wheelSteps--
            if (panel.wheelSteps > 0)
                restart()
        }
    }
    ...
    function applySelected() {
        ...
        if (!isThemeMode) panel.wallpaperCurrentImage = path
        if (isThemeMode) {
    ...
    function applyScan(text, fromCache, mode) {
        ...
        var preferredImage = panel.currentImage
        if (mode === "wallpaper" && panel.wallpaperCurrentImage !== "")
            preferredImage = panel.wallpaperCurrentImage

        var preferredIndex = Model.indexForSelectedImage(images, preferredImage)

        panel.selectedIndex = preferredIndex >= 0 ? preferredIndex : 0
    ...
    MouseArea {
        ...
        onWheel: function(wheel) {
            ...
            if (dir !== panel.wheelPendingDir) {
                panel.wheelPendingDir = dir
                panel.wheelSteps = 0
            }

            panel.wheelSteps = Math.min(panel.wheelSteps + 1, 3)
            panel.wheelTimer.restart()
        }
    }
                        ...
                        MouseArea {
                            ...
                            onClicked: {
                                if (slice.selected) {
                                    ...
                                    if (!panel.isThemeMode && panel.imageArray[index].filePath) {
                                        panel.wallpaperCurrentImage = panel.imageArray[index].filePath
                                        panel.saveModeState()
                                    }
                                }
                            }
